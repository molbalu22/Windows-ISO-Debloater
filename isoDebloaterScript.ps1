...
if (-not (Test-Path -Path $Oscdimg)) {
    ...
    New-Item -ItemType Directory -Path $OscdimgPath -Force > $null 2>&1
    New-Item -ItemType Directory -Path $ADKfolder -Force > $null 2>&1
    ...
    if ($RedirectResponse.StatusCode -eq 302) {
        ...
        Write-LogMessage "Downloading CAB file from: $CabURL"
        Invoke-WebRequest -Uri $CabURL -OutFile $CabFilePath -UseBasicParsing
    
        # Extract the CAB file
        Write-LogMessage "Extracting CAB file..."
        expand.exe -F:* $CabFilePath $ADKfolder > $null 2>&1
        ...
        if (Test-Path $ExtractedFilePath) {
            Move-Item -Path $ExtractedFilePath -Destination $FinalFilePath -Force > $null 2>&1
            Write-Host "Oscdimg.exe downloaded successfully" -ForegroundColor Green
            Write-LogMessage "Oscdimg.exe successfully placed in: $OscdimgPath"
        }
        else {
            Write-LogMessage "Error: Extracted file not found!"
        }
    }
    else {
        Write-Host "Failed to download Oscdimg.exe" -ForegroundColor Red
        Write-LogMessage "Failed to resolve ADK download link. HTTP Status: $($RedirectResponse.StatusCode)"
        Remove-TempFiles
        Exit
    }
}
